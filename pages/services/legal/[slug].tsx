...
export default function Inner({ services, service }) {
    ...
    return (
      <ServiceLayout service={service.seo}>
        <section className={`${styles.services_inner}`}>
          <div className={stylesMain.services_main__path}>
            ...
            <Link href={`/services/legal`}>
              <a className={stylesMain.services_main__path_text}>Услуги для юридических лиц &nbsp;&#62;&nbsp;&nbsp;</a>
            </Link>
            <p className={stylesMain.services_main__path_text}>{service.title}</p>
          </div>
          <div className={styles.services_inner__container}>
            <ul className={styles.services_inner__nav}>
              {
                 services.map((item)=>{
                  if(item.attributes.heading === 'legal'){
                    return ( 
                      <li key={item.id}>
                        <Link href={`/services/legal/${item.attributes.slug}`}> 
                          <a className={`${styles.services_inner__nav_item} ${item.attributes.slug == service.slug ? styles.services_inner__nav_item_active : ''}`}>
                ...
                <li>
                  <Link href={`/services/physical`}> 
                    <a className={`${styles.services_inner__nav_item}`}>
                      <div className={`${styles.services_inner__nav_item_img} ${styles.services_inner__nav_item_img_mini}`}>
                        <Image loader={() => `/services/icon-physical.svg?w=30`} src={`/services/icon-physical.svg`} width={30} height={26} alt="иконка" />
                      </div>
                      <p className={styles.services_inner__nav_item_text}>Услуги для физических лиц</p>
                    </a>
                  </Link>
                </li>
            </ul>
            <div className={styles.services_inner__content_container}>
              <div className={styles.services_inner__title_container}>
                <h1 className={styles.services_inner__title}>{service.title}</h1>
                <div className={styles.services_inner__title_img}>
              ...
              <div className={styles.services_inner__text_container}>
